...
class PasswordGenerator {
    constructor() {
        ...
        // 当前生效的单词列表（默认使用内置列表）
        this.commonWords = [...this._defaultCommonWords];

        // 从 chrome.storage.sync 加载自定义单词列表
        this._loadCustomWords();
        ...
        if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.sync) {
            chrome.storage.sync.get('commonWords', (data) => {
                if (data.commonWords && Array.isArray(data.commonWords) && data.commonWords.length > 0) {
                    this.commonWords = data.commonWords;
                    console.log('已加载自定义单词列表: ' + this.commonWords.length + '个单词');
                }
            });
        }
    }
